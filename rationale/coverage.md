...
| Handler is thin: parse/auth → call app service; no domain math/repo | `handlers.md#rules` (near-miss cross-ref: `application-services.md#is-this-what-im-building`) | — | `python.md#inbound-handlers-and-hosts` | #15 | "writing a handler / endpoint / controller" |
...
| Public interface = decoupling boundary; `Client` speaks DTOs, satisfied by embedding the service | `public-interface.md#rules` | `go.md#the-composition-root` | `python.md#the-composition-root` | #17 | "exposing a component/service behind a public interface" |
| Composition root = single wiring site; returns/injects interfaces never domain objects, chooses the impl, built once; takes a `Config` in, never reads the environment | `bootstrap.md#rules` | `go.md#the-composition-root` | `python.md#the-composition-root` | #18 | "writing the app's composition root / `main` / app config / lifecycle" |
| Cross-context gateway: the consumer owns the port; adapter in the caller's gateways; one-way deps; sync calls fail-closed | `gateway-cross-context.md#rules` | — | — (verified impl `examples/python-app`; mechanics doc pending) | — | "making one context call or read another" |
...
| — no Go analyzer (the Go serialization mirror is a named gap — `TODOS.md`) | `TB015` public-decompiler | a domain object never serializes itself (`serialization.md` rules 1/3/5), in four shapes under one code: a public method returning a spec-classified type, an emit-a-sink method handing private fields to a parameter, a second or mismatched conversion dunder on a leaf, and any conversion dunder at all on a structured type (the zero-dunder contract, collection VOs included). Deeper laundering is declared out of contract | `tb015/{good,bad}.py` |
| — no Go analyzer (same gap) | `TB016` value-object-primitives | what a value object may be built from (`serialization.md` rule 5's internal half; the 2026-07-20 R1 ruling + bool/complex amendment): a compound (≥2 fields) holds child value objects, not bare wrappable primitives; and `bool`/`complex` are not value-object material at all (a bool is atomic, complex has no wire form), so wrapping one is the violation regardless of field count. A single wrappable-primitive field is a leaf, untouched | `tb016/{good,bad}.py` |
...
| — no Go analyzer (the Go testing mirror is a named gap — `TODOS.md`) | `TB030` no-mock-libraries | testing norm v0 (`testing.md` rule 1): a test double is a hand-written fake — mocking libraries (`unittest.mock` in any import shape, the `mock` backport, pytest-mock's `mocker`) and pytest's `monkeypatch`/`MonkeyPatch` are banned tree-wide, with `# tessercheck:ignore` for a wiring test that must patch a process seam | `tb030/{good,bad}.py` |
