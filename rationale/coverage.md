...
| Handler is thin: parse/auth → call app service; no domain math/repo | `handlers.md#rules` (near-miss cross-ref: `application-services.md#is-this-what-im-building`) | — | — | #15 | "writing a handler / endpoint / controller" |
...
| Context wiring: coordinate-driven impl selection, config lives in the wiring, cross-context deps injected | `wiring.md` (stub) | — | — | — | "wiring a context's own construction / its config" |
| Hosts: one per delivery mechanism; the host is the env edge; only the edge exits; one graph per process | `srv.md` (stub) | — | — | — | "writing an entry point / server / CLI host" |
