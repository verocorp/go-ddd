// Package gclplugin registers ddd-vet's analyzers as a golangci-lint module
...
// .custom-gcl.yml that imports this package, then enable the "dddvet" linter in
// .golangci.yml. See examples/golangci/ and the README "Editor integration".
//
// Config travels unchanged: the analyzers read .go-ddd.yaml from the package
...
// does not pass analyzer flags); .go-ddd.yaml stays the single config source.
package gclplugin
...
func init() {
	register.Plugin("dddvet", New)
}

// New is the golangci-lint plugin constructor. ddd-vet takes its configuration
// from .go-ddd.yaml, not from golangci-lint settings, so settings is ignored.
...
// BuildAnalyzers returns every ddd-vet analyzer — the same
// internal/analyzers.All registry cmd/ddd-vet composes — so the plugin and the
// standalone binary can never drift.
func (p *plugin) BuildAnalyzers() ([]*analysis.Analyzer, error) {
	return analyzers.All, nil
}

// GetLoadMode requests type information: most ddd-vet analyzers are type-aware
